classdef  mirtoolboxWizard < handle
    ...
    methods
        ...
        function keyOut = unpackKey(src,keyStruct)
            ...
            if ~isempty(src.inharmonicity)

                tempNoteValue = cellReveal(src,keyStruct.key1Out);
                keyOut.value = cat(1,[src.inharmonicity.value],tempNoteValue);
                ...
                switch majorORminor
                    ...
                    case true
                       ...
                       keyOut.note = cat(1,[src.inharmonicity.note],strcat(tempNote,'M'));
                        

                    case false

                        keyOut.note = cat(1,[src.inharmonicity.note],...
                            strcat(tempNote,'m'));
                end
